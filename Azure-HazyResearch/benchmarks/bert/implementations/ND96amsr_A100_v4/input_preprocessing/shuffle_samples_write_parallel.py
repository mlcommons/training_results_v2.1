"""Copied from shuffle_samples_write.py, but we parallelize the writes to speed them up.
Otherwise it's way too slow (~30s per shard, for 4320 shards).
"""
...
p.map(process_shard, list(range(num_shards)))
