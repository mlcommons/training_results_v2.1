...
def parse_arguments():
    ...
    parser.add_argument("--fused_bias_fc_loss_head",
                        ...
                        action='store_true',
                        help="Whether to run with optimizations.")
    parser.add_argument("--fused_dense_gelu_dense",
                        default=False,
                        action='store_true',
                        help="Whether to run with optimizations.")
    parser.add_argument("--fused_dropout_add_ln",
                        default=False,
                        action='store_true',
                        help="Whether to run with optimizations.")
    parser.add_argument("--attn_split",
                        default=False,
                        action='store_true',
                        help="Whether to run with optimizations.")
    parser.add_argument("--dense_seq_output",
                        ...
                        action='store_true',
                        help="Whether to run with optimizations.")
    parser.add_argument("--last_layer_subset",
                        default=False,
                        action='store_true',
                        help="Whether to run with optimizations.")
...
def remap_fused_dense_gelu_dense_model_parameters(model_dict, config):
    res_dict = OrderedDict()
    for k in model_dict:
        if re.match('^(bert_model_segment.)?bert.encoder.layer.(\d+).intermediate.dense.(weight|bias)', k):
            new_k = re.sub(r'intermediate.dense.', 'fused_mlp.dense_gelu_dense.fc1.', k)
        elif re.match('^(bert_model_segment.)?bert.encoder.layer.(\d+).output.dense.(weight|bias)', k):
            new_k = re.sub(r'output.dense.', 'fused_mlp.dense_gelu_dense.fc2.', k)
        elif re.match('^(bert_model_segment.)?bert.encoder.layer.(\d+).output.LayerNorm.(weight|bias)', k):
            new_k = re.sub(r'output.LayerNorm.', 'fused_mlp.LayerNorm.', k)
        else:
            new_k = k
        res_dict[new_k] = model_dict[k]
    for d in range(config.num_hidden_layers):
        # Wq = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.query.weight')
        # Wk = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.key.weight')
        # Wv = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.value.weight')
        # bq = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.query.bias')
        # bk = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.key.bias')
        # bv = res_dict.pop(f'bert_model_segment.bert.encoder.layer.{d}.attention.self.value.bias')
        # res_dict[f'bert_model_segment.bert.encoder.layer.{d}.attention.self.Wqkv.weight'] = torch.cat(
        #     [Wq, Wk, Wv], dim=0
        # )
        # res_dict[f'bert_model_segment.bert.encoder.layer.{d}.attention.self.Wqkv.bias'] = torch.cat(
        #     [bq, bk, bv], dim=0
        # )
        Wq = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.query.weight')
        Wk = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.key.weight')
        Wv = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.value.weight')
        bq = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.query.bias')
        bk = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.key.bias')
        bv = res_dict.pop(f'bert.encoder.layer.{d}.attention.self.value.bias')
        res_dict[f'bert.encoder.layer.{d}.attention.self.Wqkv.weight'] = torch.cat(
            [Wq, Wk, Wv], dim=0
        )
        res_dict[f'bert.encoder.layer.{d}.attention.self.Wqkv.bias'] = torch.cat(
            [bq, bk, bv], dim=0
        )
    model_dict.clear()
    return res_dict
...
def prepare_model_and_optimizer(args, device, stream):
    ...
    config.last_layer_subset = args.last_layer_subset
    config.unpad = args.unpad
    config.attn_split = args.attn_split
    ...
    if args.init_checkpoint is not None or found_resume_checkpoint(args):
        ...
        if config.fused_dense_gelu_dense:
            # checkpoint_remapped = remap_fused_dense_gelu_dense_model_parameters(checkpoint_remapped,
            checkpoint_remapped = remap_fused_dense_gelu_dense_model_parameters(checkpoint, config)
...
#            optimizer._init_everything()
    ...
    with torch.cuda.stream(stream):
        ...
        if set_param_views_to_flat_buffer:
            p_names=[]
...
#            assert use_DDP, "Running multi-GPU without --use_ddp"
        ...
        #segmenter option, can also be used when use_cuda_graph is disabled
        ...
        model.register_forward_hook(grads_to_none_hook)
