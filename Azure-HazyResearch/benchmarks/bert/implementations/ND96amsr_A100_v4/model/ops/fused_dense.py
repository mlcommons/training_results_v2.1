...
class FusedDenseFuncTD(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, x, weight, bias, bf16=False):
        if torch.is_autocast_enabled():
            dtype = torch.get_autocast_gpu_dtype() if not bf16 else torch.bfloat16
    ...
    def backward(ctx, grad_output):
        ...
        return grad_input.reshape_as(x), grad_weight, grad_bias, None
...
class FusedDenseTD(nn.Linear):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, bf16=False,
    ...
    def forward(self, x):
        if x.is_cuda and self.bias is not None:
            return fused_dense_function_td(x, self.weight, self.bias, self.bf16)
        else:
...
class FusedDenseGeluDenseFuncTD(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, x, weight1, bias1, weight2, bias2, checkpoint_lvl=0, bf16=False, heuristic=0):
        """checkpoint_lvl:
    ...
    def backward(ctx, grad_output):
        ...
        #     x.reshape(batch_dim, n), weight1, grad_gelu
        # )
        return grad_input.reshape_as(x), grad_weight1, grad_bias1, grad_weight2, grad_bias2, None, None, None
...
class FusedDenseGeluDenseTD(nn.Module):
    ...
    def __init__(self, in_features, intermediate_features, out_features=None, bias=True,
                 checkpoint_lvl=0, bf16=False, heuristic=0, device=None, dtype=None):
        """
    ...
    def forward(self, x):
        return fused_dense_gelu_dense_function_td(x, self.fc1.weight, self.fc1.bias,
                                                  ...
                                                  self.checkpoint_lvl, self.bf16, self.heuristic)
