...
from scaleoutbridge import init_bridge, EmptyObject, ScaleoutBridge as SBridge
...
def train_step( model, loss_fn, args, batch_size, feats, feat_lens, txt, txt_lens, optimizer, grad_scaler, 
                meta_data, train_loader, rnnt_graph, copy_stream, pred_stream, sbridge=EmptyObject()):
    rank = dist.get_rank()
...
def main():
    ...
    SUBMISSION_PLATFORM_PLACEHOLDER = "G5500-V6"
    logging.log_event(logging.constants.SUBMISSION_BENCHMARK, value=logging.constants.RNNT)
    logging.log_event(logging.constants.SUBMISSION_ORG, value='xFusion')
 ...
 #   logging.log_event(logging.constants.SUBMISSION_PLATFORM, value=f'{num_nodes}xSUBMISSION_PLATFORM_PLACEHOLDER')
     logging.log_event(logging.constants.SUBMISSION_PLATFORM,value=f'{num_nodes}x{SUBMISSION_PLATFORM_PLACEHOLDER}')
