...
DALI_EVAL_HW_DECODER_LOAD=${DALI_EVAL_HW_DECODER_LOAD:-'0.0'}
WD=${WD:-'0.0001'}
...
DALI_EVAL_PREFETCH_QUEUE=${DALI_EVAL_PREFETCH_QUEUE:-2}
DALI_NVJPEG_MEMPADDING=${DALI_NVJPEG_MEMPADDING:-64}
DALI_THREADS=${DALI_THREADS:-3}
DALI_EVAL_THREADS=${DALI_EVAL_THREADS:-3}
...
if [[ "${KVSTORE}" == "horovod" ]]; then
    ...
    if [[ "${DGXSYSTEM}" == NF5488A5 ]]; then
        BIND="./bind.sh --cpu=nf5488a5_cxx.sh --mem=nf5488a5_cxx.sh --cluster=${cluster} --"
    elif [[ "${DGXSYSTEM}" == NF5688M6 ]]; then
        BIND="./bind.sh --cpu=nf5688m6_cxx.sh --mem=nf5688m6_cxx.sh --cluster=${cluster} --"
    elif [[ "${DGXSYSTEM}" == NF5468M6J ]]; then
        BIND="./bind.sh --cpu=nf5468m6j_cxx.sh --mem=nf5468m6j_cxx.sh --cluster=${cluster} --"
	#g5500v6_htx1.sh
    elif [[ "${DGXSYSTEM}" == G5500V6x8A100-PCIE-80GB ]]; then
	 BIND=( './bind.sh' '--' ${NSYSCMD} 'python' '-u' )  
  #      BIND="./bind.sh --cpu=g5500v6_htx1.sh --mem=g5500v6_htx1.sh --cluster=${cluster} --"
    else 
