...
readonly _logfile_base="${LOGDIR}/result"

readonly _cont_name=image_classification
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=$(pwd):/workspace/image_classification")
# "--volume=$(pwd):/workspace/image_classification"
# MLPerf vars
MLPERF_HOST_OS=$(
...
mlperf_log_utils.mx_resnet_print_event(key=constants.CACHE_CLEAR, val=True)"
	  ...
	  #  tee "${_logfile_base}_${_experiment_index}.log"
             tee "${_logfile_base}_${_experiment_index}.txt"
done
