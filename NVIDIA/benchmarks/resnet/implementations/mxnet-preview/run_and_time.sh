...
if [[ "${CONTAINER_UPDATED_LIBS}" ]]; then
    export LD_LIBRARY_PATH="${CONTAINER_UPDATED_LIBS}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
fi
echo LD_LIBRARY_PATH="${LD_LIBRARY_PATH}"
echo __RM_NO_VERSION_CHECK="${__RM_NO_VERSION_CHECK}"
...
if [ ${NVTX_FLAG} -gt 0 ]; then
 NSYSCMD=" nsys profile --capture-range cudaProfilerApi --capture-range-end stop --sample=none --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output /results/image_classification_mxnet_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}_${SYNTH_DATA}_${DISABLE_CG}.nsys-rep "
else
 NSYSCMD=""
fi
