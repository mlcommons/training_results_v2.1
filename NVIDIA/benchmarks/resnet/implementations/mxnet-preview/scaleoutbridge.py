...
class ScaleoutBridge(object):
    ...
    def print_tag(self, tag, dur):
        mx_resnet_print_event(key=tag, val={'r':self.deviceid, 't':dur}, uniq=False)
    ...
    def printstat(self, tag):
        ...
        mx_resnet_print_event(key=tag+'_total', val={'r':self.deviceid, 't':total}, uniq=False)
        mx_resnet_print_event(key=tag+'_count', val={'r':self.deviceid, 't':count}, uniq=False)
        mx_resnet_print_event(key=tag+'_min', val={'r':self.deviceid, 't':minimum}, uniq=False)
        mx_resnet_print_event(key=tag+'_max', val={'r':self.deviceid, 't':maximum}, uniq=False)
...
class ScaleoutBridge_Epoch(object):
    ...
    def stop_epoch_prof(self):
        mx.nd.waitall()
        mx_resnet_print_event(key='epoch_time', val={'r':self.deviceid, 't':time()-self.start_time}, uniq=False)
    def start_eval_prof(self):
        pass
    def stop_eval_prof(self):
        return 1
