...
def mlperf_fit(self, args, train_data,
    ...
    for epoch in range(begin_epoch, num_epoch):
        ...
        mx_resnet_print_start(key=constants.EPOCH_START, metadata={'epoch_num': epoch + 1})
        tic = time.time()
        eval_metric.reset()
        nbatch = 0
        ...
        mx_resnet_print_event(key='tracked_stats',
                              val={'throughput': speed},
                              metadata={'step': (epoch + 1)})
        mx_resnet_print_event(key='throughput',
                              val=speed)
        ...
        if eval_data and epoch % eval_period == eval_offset:
            ...
            mx_resnet_print_start(key=constants.EVAL_START, metadata={'epoch_num': epoch + 1})
            ...
            mx_resnet_print_end(key=constants.BLOCK_STOP,
        ...
        if not args.input_batch_multiplier > 1:
            train_data.reset()
        sbridge.stop_epoch_prof()
...
def fit(args, kv, model, initializer, data_loader, devs, arg_params, aux_params, **kwargs):
    ...
    mx_resnet_print_event(key='d_batch_size', val=args.batch_size)
    mx_resnet_print_event(key='s_optimizer', val=args.optimizer)
    mx_resnet_print_event(key='s_network', val=args.network)
    mx_resnet_print_event(key='s_process', val=args.kv_store)
    mx_resnet_print_event(key=constants.GLOBAL_BATCH_SIZE, val=args.batch_size * num_workers)
    mx_resnet_print_event(key='s_optimizer', val=args.optimizer)
    mx_resnet_print_event(key='s_network', val=args.network)
    mx_resnet_print_event(key='s_process', val=args.kv_store)
    mx_resnet_print_event(key=constants.GRADIENT_ACCUMULATION_STEPS, val=1)
    ...
    if args.optimizer in {'sgdwlars', 'sgdwfastlars','sgdwfastlarsv2'}:
        ...
        mx_resnet_print_event(key=constants.OPT_NAME,
                              val='lars')
        mx_resnet_print_event(key=constants.LARS_EPSILON,
                              val=args.lars_eps)
        mx_resnet_print_event(key=constants.LARS_OPT_WEIGHT_DECAY,
                              val=args.wd)
        mx_resnet_print_event(key='lars_opt_momentum', val=optimizer_params['momentum'])
        mx_resnet_print_event(key='lars_opt_base_learning_rate', val=optimizer_params['learning_rate'])
        mx_resnet_print_event(key='lars_opt_learning_rate_warmup_epochs', val=args.warmup_epochs)
        mx_resnet_print_event(key=constants.LARS_OPT_LR_DECAY_STEPS, val=args.num_epochs)
        ...
        mx_resnet_print_event(
            key=constants.OPT_NAME,
            val='sgd')
        mx_resnet_print_event(
            key='sgd_opt_weight_decay',
            val=args.wd)
        mx_resnet_print_event(key='sgd_opt_momentum', val=optimizer_params['momentum'])
