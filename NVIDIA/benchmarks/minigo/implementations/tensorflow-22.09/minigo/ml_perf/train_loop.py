...
def run_train(state, rank, tstate, tcomm, tzcomm):
    ...
    # pause selfplay
    if rank == 0:
        pauseflag_path = os.path.join(FLAGS.export_path, state.train_model_name) + '.pauseflag'
        with open(pauseflag_path, 'w') as flag:
            pass
...
def selfplay(rank):
    ...
    pausing = rank < FLAGS.ranks_per_node and rank % gpus_per_node < FLAGS.num_gpus_train;
...
def mlperf_log_hyperparams(rank):
    if rank == 0:
        ...
        log_event(key='d_batch_size', value=FLAGS.train_batch_size, sync=False)
        ...
        while state.iter_num <= FLAGS.iterations:
            ...
            if rank == 0:
              log_event(key='tracked_stats',
                        value= {'throughput' : (state.iter_num - checkpoint_model_num)/(time.time() - state.start_time)},
                        metadata={"step": (state.iter_num)})
              log_event(key='throughput',
                        value= (state.iter_num - checkpoint_model_num)/(time.time() - state.start_time))
