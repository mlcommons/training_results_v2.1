...
def preprocess_batch(args, input_ids, segment_ids, input_mask, labels_mlm, labels_nsp,  packed_seqlens=None, input_only=False, get_ntokens=False):
    b, s = input_ids.shape
    if args.pad_fmha:
...
class FwdLossBwdTrainer():
    ...
    def step(self, step, batch, model, optimizer, sbridge=SBridge(), ntokens=None, data_iter=None):
        loss = None
        mlm_acc = None
        ...
        if data_iter is not None:
            ...
            if next_batch is not None:
                next_batch = preprocess_batch(self.args, *next_batch, get_ntokens=(ntokens is not None))
