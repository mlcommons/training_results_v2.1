...
class BertTransformerLayer2(nn.Module):
    def __init__(self, config):
        ...
        print ('Init BertTransformerLayer')
        ...
        self.output.LayerNorm._register_load_state_dict_pre_hook(remove_extra_states)

    def forward(self, hidden_states, attention_mask, seqlen=None, ntokens=None):
        attention_output = self.attention(hidden_states, attention_mask, seqlen, ntokens)
        hidden_states = fused_dropout_add(attention_output, hidden_states, self.p, self.training)
        intermediate_output, hidden_states = self.layernorm_mlp(hidden_states)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, checkpoint_activations=False):
        ...
        elif seqlen is None and batch is None and self.use_transformer_engine2:
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, masked_lm_labels, masked_lm_labels_dense_idx=None):
        if self.dense_seq_output:
            ...
            mlm_idx = torch.nonzero(masked_lm_labels.view(-1) != 0, as_tuple=False).squeeze() \
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, output_all_encoded_layers=True, checkpoint_activations=False):
        ...
        pooled_output = self.pooler(sequence_output, extended_attention_mask)
...
class BertForPreTrainingModelOnly(nn.Module):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, checkpoint_activations=False):
...
class BertForPreTrainingSegmented(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dense_seq_output = config.dense_seq_output
        self.apply(self.init_bert_weights)
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None, position_ids=None, checkpoint_activations=False):
        ...
        if position_ids is None:
            ...
            sequence_output, pooled_output = self.bert_model_segment(input_ids, token_type_ids, attention_mask, position_ids)
        return self.heads_only_segment(sequence_output, pooled_output, masked_lm_labels, next_sentence_label, masked_lm_labels_dense_idx)
