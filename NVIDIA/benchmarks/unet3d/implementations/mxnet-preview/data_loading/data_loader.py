...
def get_data_split(path: str):
    ...
    mllogger.event(key='train_samples', value=len(imgs_train), sync=False)
    mllogger.event(key='eval_samples', value=len(imgs_val), sync=False)
...
def get_data_loaders(flags, data_dir, seed, local_rank, global_rank, train_ranks, eval_ranks,
                     spatial_group_size, eval_mode, ctx, world_size):
    ...
    if global_rank in eval_ranks:
        if eval_mode == "sharded":
            ...
            num_shards = len(eval_ranks)
            shard_id = (global_rank - eval_ranks[0])
            assert len(eval_ranks) <= 42, f"Too many eval ranks {len(eval_ranks)}"
        else:
            raise ValueError(f"Wrong eval_mode {eval_mode}")

        x_val, y_val = make_val_split_even(x_val, y_val, num_shards=num_shards, shard_id=shard_id,
                                           shard_eval=eval_mode == "sharded", batch_size=flags.val_batch_size,
