...
def sync_training_and_evaluation(global_comm, transfer_comm, rank, model, eval_ranks, transfer_ranks, stop_training,
    ...
    local_stop_training = np.array([stop_training, diverged], dtype=np.int32)
    global_stop_training = np.zeros(2, dtype=np.int32)
    # local_diverged = np.array([stop_training], dtype=np.int32)
    # global_diverged = np.zeros(1, dtype=np.int32)
    global_comm.Allreduce(local_stop_training, global_stop_training, MPI.SUM)
    # global_comm.Allreduce(local_diverged, global_diverged, MPI.SUM)
    # t1 = time()
