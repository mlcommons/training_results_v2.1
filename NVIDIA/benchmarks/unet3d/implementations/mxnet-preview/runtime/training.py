...
def train(flags, model, train_loader, val_loader, score_fn, sw_inference,
          ...
          transfer_ranks, ctx, callbacks, mllogger):
    rank = comm.Get_rank()
    stop_training = False
    diverged = False
    ...
    for cycle in range(1, max_cycles):
        ...
        if (global_epoch >= flags.start_eval_at) and flags.nodes_for_eval:
            ...
            stop_training, diverged, model = sync_training_and_evaluation(comm, transfer_comm, rank, model, eval_ranks,
                                                                          transfer_ranks, stop_training, diverged)
            sbridge.stop_eval_prof()

        if stop_training or diverged:
            break
    ...
    if stop_training and not diverged:
