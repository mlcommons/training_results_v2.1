...
def check_flags(flags, world_size):
    ...
    if flags.spatial_group_size > 1:
        ...
        if flags.eval_mode == "spatial":
            assert flags.val_batch_size == 1, f"val_batch_size must be equal to 1, got {flags.val_batch_size}"
    # if 168 % (flags.batch_size * world_size) and flags.stick_to_shard:
    #     raise ValueError("Stick to shard cannot be used while 168 is not divisible by GBS. "
    #                      "Remove -sts or --stick_to_shard from the command line")
