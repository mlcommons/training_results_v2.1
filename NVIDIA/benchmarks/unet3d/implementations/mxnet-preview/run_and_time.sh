...
if [ ${NVTX_FLAG} -gt 0 ]; then
 NSYSCMD=" nsys profile --capture-range cudaProfilerApi --capture-range-end stop --sample=none --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output /results/image_segmentation_mxnet_${DGXNNODES}x${DGXNGPU}x${BATCH_SIZE}_${DATESTAMP}_${SLURM_PROCID}_${SYNTH_DATA}_${DISABLE_CG}.nsys-rep "
else
 NSYSCMD=""
fi
...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    CMD=( 'bindpcie' '--ib=single' '--cpu=exclusive' '--' ${PROFILING_PREFIX} ${NSYSCMD} 'python' '-u' )
else
