...
class SpatialNetwork(gluon.HybridBlock):
    def __init__(self, n_classes, spatial_group_size, local_rank, comm):
        ...
        with self.name_scope():
            self.input_split = SplitBlock(spatial_group_size=spatial_group_size, comm=comm)
...
class Unet3D(gluon.HybridBlock):
    def __init__(self, n_classes, spatial_group_size, local_rank, comm, is_eval, eval_mode, global_rank):
        super(Unet3D, self).__init__()
        self.channel_axis = -1
        self.trainer = None
        self.dummy_trainer = None
        if spatial_group_size == 1 or (is_eval and eval_mode != "spatial"):
    ...
    def init(self, flags, ctx, world_size, steps_per_epoch, is_training_rank, cold_init=True, warmup_iters=0):
        ...
        self.network.hybridize(static_alloc=True,
                               static_shape=(flags.spatial_group_size == 1) or flags.eval_mode != "spatial")
        if world_size > 1:
