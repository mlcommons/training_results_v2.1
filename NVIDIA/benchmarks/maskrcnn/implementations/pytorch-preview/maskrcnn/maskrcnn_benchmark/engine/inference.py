...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu, dedicated_evaluation_ranks, eval_ranks_comm):
    ...
    all_predictions = all_gather(predictions_per_gpu, world_size, eval_ranks_comm)
...
def inference(
        ...
        output_folder=None,
        eval_segm_numprocs=1,
        eval_mask_virtual_paste=False,
        dedicated_evaluation_ranks=0,
        eval_ranks_comm=None,
        dryrun=False,
):
    # convert to a torch.device for efficiency
    device = torch.device(device)
    if torch.distributed.is_initialized():
        num_evaluation_ranks = torch.distributed.get_world_size() if dedicated_evaluation_ranks == 0 else dedicated_evaluation_ranks
    else:
        num_evaluation_ranks = 1
    logger = logging.getLogger("maskrcnn_benchmark.inference")
    ...
    synchronize(eval_ranks_comm)
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=total_time))
    logger.info(
        "Total inference time: {} ({} s / img per device, on {} devices)".format(
            total_time_str, total_time * num_evaluation_ranks / len(dataset), num_evaluation_ranks
        )
    )

    # During dryrun, load images and push them through model, but skip evaluation.
    if dryrun:
        return
