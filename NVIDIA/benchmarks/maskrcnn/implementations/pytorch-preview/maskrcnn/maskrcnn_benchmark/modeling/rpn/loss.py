...
class RPNLossComputation(object):
    ...
    def __call__(self, anchors, objectness, box_regression, targets):
        ...
        if self.syncfree and N == 1:
            ...
            box_regression_index_select = box_regression.index_select(0, sampled_inds) * sampled_pos_inds_mask
            with torch.cuda.stream(stream1):
                regression_targets_index_select = regression_targets.index_select(0, sampled_inds) * sampled_pos_inds_mask
            current_stream.wait_stream(stream1)
            box_loss = smooth_l1_loss(
                box_regression_index_select,
                regression_targets_index_select,
                beta=1.0 / 9,
                size_average=False,
            ) / counts[1]
        else:
            ...
            stream0.wait_stream(current_stream)
            with torch.cuda.stream(stream0):
                sampled_inds = torch.cat([sampled_pos_inds, sampled_neg_inds], dim=0)
            sampled_inds_numel = sampled_pos_inds.shape[0] + sampled_neg_inds.shape[0]
            stream1 = torch.cuda.Stream()
            stream1.wait_stream(current_stream)
            box_regression_index_select = box_regression.index_select(0, sampled_pos_inds)
            with torch.cuda.stream(stream1):
                regression_targets_index_select = regression_targets.index_select(0, sampled_pos_inds)
            current_stream.wait_stream(stream1)
            box_loss = smooth_l1_loss(
                box_regression_index_select,
                regression_targets_index_select,
