...
class ScaleoutBridge(object):
    ...
    def print_tag(self, tag, dur):
        mllogger.event(key=tag, value={'r':self.deviceid, 't':dur}, log_all_ranks=True)
    ...
    def printstat(self, tag):
        ...
        mllogger.event(key=tag+'_total', value={'r':self.deviceid, 't':total}, log_all_ranks=True)
        mllogger.event(key=tag+'_count', value={'r':self.deviceid, 't':count}, log_all_ranks=True)
        mllogger.event(key=tag+'_min', value={'r':self.deviceid, 't':minimum}, log_all_ranks=True)
        mllogger.event(key=tag+'_max', value={'r':self.deviceid, 't':maximum}, log_all_ranks=True)
...
class ScaleoutBridge_Epoch(object):
    ...
    def stop_epoch_prof(self):
        torch.cuda.synchronize()
        mllogger.event(key='epoch_time', value={'r':self.deviceid, 't':time()-self.start_time}, log_all_ranks=True)
    def start_eval_prof(self):
        pass
    def stop_eval_prof(self):
        pass
