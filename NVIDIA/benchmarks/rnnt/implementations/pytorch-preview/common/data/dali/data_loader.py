...
class DaliDataLoader:
    ...
    def __init__(self, gpu_id, dataset_path: str, shuffle, config_data: dict, config_features: dict,
                 ...
                 pre_sort=False, dont_use_mmap=False, gpu_resampling=False):
    ...
    def fetch_next(self):
        ...
        if self.pipeline_type == 'train':
            # use async copy as we don't really want to sync here if prefetching
            meta_data = self.preproc.get_meta_data(
                max_f_len,
                audio_len,
                transcripts,
                transcripts_len,
                async_cp=self.enable_prefetch,
            )
        else:
            meta_data = None

        return audio, audio_len, transcripts, transcripts_len, meta_data
    ...
    def __next__(self):
        if self.enable_prefetch:
            ...
            if self.prefetched_data is None:
                ...
                batch = self.prefetched_data
                self.preproc.copy_metadata(batch[-1])
        else:
            batch = self.fetch_next()

        audio, audio_len, transcripts, transcripts_len, meta_data = batch
