...
class Preproc():
    ...
    def copy_metadata(self, meta_data):
        # write asyncly copied packed batch info into meta data
        for meta, packed_batch_cpu in zip(meta_data, self.list_packed_batch_cpu):
            meta['packed_batch'] = packed_batch_cpu.item()

    def get_meta_data(self, max_f_len, audio_shape_, transcripts, transcripts_lengths, async_cp=False):
        meta_data = []
        B_split = transcripts.size(0) // self.batch_split_factor
        for i in range(self.batch_split_factor):
            meta_data.append(self.get_packing_meta_data(max_f_len,
    ...
    def get_packing_meta_data(self, max_f_len, feat_lens, txt_lens, async_cp=False, idx=0):
        ...
        if self.apex_transducer_joint is not None:
            g_len = txt_lens + 1
            if self.apex_transducer_joint.startswith("pack"):
                batch_offset = torch.cumsum(g_len * torch.div(feat_lens + self.enc_stack_time_factor - 1,
                                                              self.enc_stack_time_factor, rounding_mode='trunc'), dim=0)
                if async_cp:
