...
from common.seeds import setup_seeds, set_system_seeds
...
def parse_args():
    ...
    training.add_argument('--apex_transducer_joint', default=None, type=str, choices=['pack', 'not_pack', 'pack_w_relu_dropout'], 
    ...
    training.add_argument('--fc_impl', default=None, type=str, choices=['apex_mlp', 'apex_fused_dense'], 
                            help='The implementation of the FC layer in the joint network')
    training.add_argument('--delay_encoder', action='store_true',
                            help='Encode after prediction during model forward')
    ...
    optim.add_argument('--dwu_overlap_reduction', action='store_true',
                       help='whether overlap allreduce with bprop')
    ...
    io.add_argument('--audio_resampling_device', type=str, choices=['cpu', 'gpu'],
                    default='cpu', help='Device to use for audio resampling')
    ...
    io.add_argument('--disable_file_logging', action='store_true',
                    help='Disable logging tensorboard and nvlog.json')
...
def evaluate(epoch, step, val_loader, val_feat_proc, detokenize,
    ...
    mllogger.start(mllogger.constants.EVAL_START, metadata=dict(epoch_num=epoch))
    ...
    mllogger.event(mllogger.constants.EVAL_ACCURACY, value=wer, metadata=dict(epoch_num=epoch))
    mllogger.end(mllogger.constants.EVAL_STOP, metadata=dict(epoch_num=epoch))
...
def train_step( model, loss_fn, args, batch_size, feats, feat_lens, txt, txt_lens, optimizer, grad_scaler, 
                meta_data, train_loader, rnnt_graph, copy_stream, pred_stream, sbridge=SBridge()):
    ...
    if args.batch_split_factor == 1:
        ...
        return loss_cpu.item(), lr_cpu.item(), sbridge

    else:
        sbridge.stop_start_prof(SBridge.FWD_TIME, SBridge.FBWD_TIME)
        ...
        sbridge.stop_start_prof(SBridge.FBWD_TIME, SBridge.BWD_TIME)
        f.backward(f_2.grad)
        g.backward(g_2.grad)
        sbridge.stop_prof(SBridge.BWD_TIME)
        return loss_item, lr_cpu.item(), sbridge
...
def main():
    ...
    if args.seed is not None:
        # NOTE: here args.seed might be different in each worker.
        #  Proper seeds are set later after initializing DistLAMB.
        #  Here we set the seed only to ensure deterministic model
        #  initialization *on rank 0*
        set_system_seeds(args.seed)
    ...
    mllogger.event(mllogger.constants.GRADIENT_ACCUMULATION_STEPS, value=args.grad_accumulation_steps)
    ...
    mllogger.mlperf_submission_log(mllogger.constants.RNNT)
    ...
    mllogger.event(mllogger.constants.MODEL_WEIGHTS_INITIALIZATION_SCALE, value=args.weights_init_scale)
    ...
    if args.fc_impl:
        rnnt_config["fc_impl"] = args.fc_impl
    rnnt_config["delay_encoder"] = args.delay_encoder
    enc_stack_time_factor = rnnt_config["enc_stack_time_factor"]
    model = RNNT(n_classes=tokenizer.num_labels + 1, **rnnt_config)
    model.cuda()
    blank_idx = tokenizer.num_labels
    assert args.apex_transducer_loss is not None, "WarpRNNTLoss no longer supported"
    ...
    loss_fn = apexTransducerLoss(   blank_idx, args.apex_transducer_loss,
                                    packed_input=args.apex_transducer_joint is not None
                                        and args.apex_transducer_joint.startswith("pack"))

    # set up evaluation
    mllogger.event(mllogger.constants.EVAL_MAX_PREDICTION_SYMBOLS, value=args.max_symbol_per_sample)
    ...
    if args.ema > 0:
        ...
        ema_model = None
    mllogger.event(mllogger.constants.MODEL_EVAL_EMA_FACTOR, value=args.ema)
    ...
    if args.dist_lamb:
        ...
        kw = {  'params': model.parameters(), 
                ...
                'overlap_reductions': args.dwu_overlap_reduction,
    ...
    # Seeds broadcast must happen after initializing DistributedFusedLAMB
    if args.seed is not None:
        mllogger.event(mllogger.constants.SEED, value=args.seed, unique=True)
        worker_seeds, shuffling_seeds = setup_seeds(args.seed, 1, torch.device('cuda'))
        worker_seed = worker_seeds[dist.get_rank() if multi_gpu else 0]
        set_system_seeds(worker_seed)
        # sampler_seed is used for buckets generation, and needs to be the same on every worker
        sampler_seed = shuffling_seeds[0]
        dali_seed = worker_seed
    ...
    mllogger.event(mllogger.constants.DATA_TRAIN_MAX_DURATION,
                      value=train_dataset_kw['max_duration'])
    mllogger.event(mllogger.constants.DATA_SPEED_PERTURBATON_MAX,
                      value=train_dataset_kw['speed_perturbation']['max_rate'])
    mllogger.event(mllogger.constants.DATA_SPEED_PERTURBATON_MIN,
                      value=train_dataset_kw['speed_perturbation']['min_rate'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_FREQ_N,
                      value=train_specaugm_kw['freq_masks'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_FREQ_MIN,
                      value=train_specaugm_kw['min_freq'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_FREQ_MAX,
                      value=train_specaugm_kw['max_freq'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_TIME_N,
                      value=train_specaugm_kw['time_masks'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_TIME_MIN,
                      value=train_specaugm_kw['min_time'])
    mllogger.event(mllogger.constants.DATA_SPEC_AUGMENT_TIME_MAX,
                      value=train_specaugm_kw['max_time'])
    mllogger.event(mllogger.constants.GLOBAL_BATCH_SIZE,
    ...
    mllogger.event(mllogger.constants.DATA_TRAIN_NUM_BUCKETS, value=args.num_buckets)
    ...
    assert args.audio_resampling_device in ('cpu', 'gpu'), \
        f'Invalid audio resampling device: {args.audio_resampling_device}'
    assert (args.dali_device, args.audio_resampling_device) != ('cpu', 'gpu'), \
        f'Cannot use GPU resampling with a CPU DALI pipeline'
    train_loader = DaliDataLoader(gpu_id=args.local_rank,
                                  ...
                                  gpu_resampling=args.audio_resampling_device == 'gpu')
    ...
    mllogger.event(mllogger.constants.TRAIN_SAMPLES, value=train_loader.dataset_size)
    mllogger.event(mllogger.constants.EVAL_SAMPLES, value=val_loader.dataset_size)

    mllogger.event(mllogger.constants.OPT_NAME, value='lamb')
    mllogger.event(mllogger.constants.OPT_BASE_LR, value=args.lr)
    mllogger.event(mllogger.constants.OPT_LAMB_EPSILON, value=opt_eps)
    mllogger.event(mllogger.constants.OPT_LAMB_LR_DECAY_POLY_POWER, value=args.lr_exp_gamma)
    mllogger.event(mllogger.constants.OPT_LR_WARMUP_EPOCHS, value=args.warmup_epochs)
    mllogger.event(mllogger.constants.OPT_LAMB_LR_HOLD_EPOCHS, value=args.hold_epochs)
    mllogger.event(mllogger.constants.OPT_LAMB_BETA_1, value=args.beta1)
    mllogger.event(mllogger.constants.OPT_LAMB_BETA_2, value=args.beta2)
    mllogger.event(mllogger.constants.OPT_GRADIENT_CLIP_NORM, value=args.clip_norm)
    mllogger.event(mllogger.constants.OPT_LR_ALT_DECAY_FUNC, value=True)
    mllogger.event(mllogger.constants.OPT_LR_ALT_WARMUP_FUNC, value=True)
    mllogger.event(mllogger.constants.OPT_LAMB_LR_MIN, value=args.min_lr)
    mllogger.event(mllogger.constants.OPT_WEIGHT_DECAY, value=args.weight_decay)
    ...
    def buffer_pre_allocation():
        max_seq_len = math.ceil(train_preproc.audio_duration_to_seq_len(
                        ...
                        * cfg["input_train"]["audio_dataset"]["speed_perturbation"]["max_rate"])
        max_txt_len = train_loader._dali_data_iterator.transcript_lens.max()
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        mllogger.start(mllogger.constants.BLOCK_START,
                          metadata=dict(first_epoch_num=epoch,
                                        epoch_count=1))
        mllogger.start(mllogger.constants.EPOCH_START,
        ...
        for batch in train_loader:
            ...
            if args.dist_lamb:
                update_step = (accumulated_batches % args.grad_accumulation_steps == 0)
                optimizer.set_is_accumulation_step(not update_step)
                optimizer.set_last_step(step == len(train_loader) - 1)

            feats, feat_lens, txt, txt_lens, meta_data = batch

            if args.enable_seq_len_stats:
                all_feat_lens += feat_lens

            loss_item, lr_item, sbridge = train_step( model, loss_fn, args, batch_size, feats, feat_lens,
                                                      txt, txt_lens, optimizer, grad_scaler, meta_data,
                                                      train_loader, rnnt_graph, copy_stream, pred_stream,
                                                      sbridge=sbridge)
            ...
            if accumulated_batches % args.grad_accumulation_steps == 0:
                ...
                if not sbridge.stop_prof(SBridge.OPT_TIME):
                ...
                # end of step
            sbridge.stop_prof(SBridge.ITER_TIME)
        mllogger.end(mllogger.constants.EPOCH_STOP,
                     metadata=dict(epoch_num=epoch))
        ...
        mllogger.event(key='tracked_stats', metadata={'step': epoch},
                       value={"throughput": epoch_utts / epoch_time})
        ...
        mllogger.end(mllogger.constants.BLOCK_STOP, metadata=dict(first_epoch_num=epoch))
