...
if [ ${NVTX_FLAG--1} -gt 0 ]; then
 NSYSCMD=" nsys profile --sample=none --duration=240 --kill=2 --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output $OUTPUT_DIR/rnn_speech_recognition_pytorch_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}_${SYNTH_DATA}.nsys-rep "
else
 NSYSCMD=""
fi
