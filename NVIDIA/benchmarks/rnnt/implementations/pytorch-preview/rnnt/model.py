...
class StackTime(nn.Module):
    ...
    def forward(self, x, x_lens):
        ...
        if type(x) is not list:
            x = self.stack(x)
            x_lens = torch.div(x_lens.int() + self.factor - 1,
                               self.factor, rounding_mode='trunc')
            return x, x_lens
        else:
            # seq splitting path
            if len(x) != 2:
...
class ReluDropout(nn.Module):
    def __init__(self, apex_transducer_joint, fuse_relu_dropout, dropout_prob):
        super().__init__()
        self.apex_transducer_joint = apex_transducer_joint
        self.dropout_prob = dropout_prob
        if apex_transducer_joint=='pack_w_relu_dropout':
            self.relu_dropout = torch.nn.Identity()
        elif fuse_relu_dropout:
            self.relu_dropout = FusedReluDropout(dropout_prob)
        else:
...
class Joint(nn.Module):
    def __init__(self, apex_transducer_joint, dropout_prob):
        super().__init__()
        self.apex_transducer_joint = apex_transducer_joint
        if self.apex_transducer_joint is not None:
            self.transducer_joint = TransducerJoint(
                                        pack_output=apex_transducer_joint.startswith('pack'),
                                        relu=apex_transducer_joint=='pack_w_relu_dropout',
                                        dropout=apex_transducer_joint=='pack_w_relu_dropout',
    ...
    def forward(self, f, g, f_len, dict_meta_data):
        ...
        if self.apex_transducer_joint is None:
            ...
            # APEX transducer_joint
            if self.training:
                h = self.transducer_joint(  f, g, f_len, dict_meta_data["g_len"], 
                                            dict_meta_data["batch_offset"], 
                                            dict_meta_data["packed_batch"])  
            else:
                # For eval, don't to packing for simplicity as it doesn't make any difference
                pack_output_status = self.transducer_joint.pack_output
                self.transducer_joint.pack_output = False
                h = self.transducer_joint(f, g, f_len, f_len)
                self.transducer_joint.pack_output = pack_output_status
        return h
...
class RNNT(nn.Module):
    ...
    def __init__(self, n_classes, in_feats, enc_n_hid,
                 ...
                 min_lstm_bs=8, fc_impl=None, delay_encoder=False):
        ...
        mllogger.log_weights_initialization('pred_embed')
        ...
        self.joint_pred = torch.nn.Linear(
            pred_n_hid,
            joint_n_hid)
        mllogger.log_weights_initialization('joint_pred')
        self.joint_enc = torch.nn.Linear(
            enc_n_hid,
            joint_n_hid)
        mllogger.log_weights_initialization('joint_enc')

        ### Joint Network
        self.transducer_joint = Joint(apex_transducer_joint, joint_dropout)
        self.relu_dropout = ReluDropout(apex_transducer_joint, fuse_relu_dropout, joint_dropout)
        ### final FC layer 
        if fc_impl == "apex_mlp":
            # make sure we use the same weight initialization 
            linear_dummy = torch.nn.Linear(joint_n_hid, n_classes)
            self.fc = MLPWrapper(joint_n_hid, n_classes)
            with torch.no_grad():
                self.fc.mlp.weights[0].copy_(linear_dummy.weight)
                self.fc.mlp.biases[0].copy_(linear_dummy.bias)
            del linear_dummy
        elif fc_impl == "apex_fused_dense":
            linear_dummy = torch.nn.Linear(joint_n_hid, n_classes)
            self.fc = FusedDenseWrapper(joint_n_hid, n_classes)
            with torch.no_grad():
                self.fc.fused_dense.weight.copy_(linear_dummy.weight)
                self.fc.fused_dense.bias.copy_(linear_dummy.bias)
            del linear_dummy
        else:
            self.fc = torch.nn.Linear(joint_n_hid, n_classes)
        mllogger.log_weights_initialization('joint_net')

        self.min_lstm_bs = min_lstm_bs
        self.delay_encoder = delay_encoder

    def forward(self, x, x_lens, y, y_lens, dict_meta_data=None, state=None):
        # x: (B, channels, features, seq_len)
        y = label_collate(y)
        if not self.delay_encoder:
            f, x_lens = self.encode(x, x_lens)

        g, _ = self.predict(y, state)

        if self.delay_encoder:
            f, x_lens = self.encode(x, x_lens)
        out = self.joint(f, g, x_lens, dict_meta_data)
    ...
    def joint(self, f, g, f_len=None, dict_meta_data=None):
        """
        ...
        """
        h = self.transducer_joint(f, g, f_len, dict_meta_data)
...
class MLPWrapper(nn.Module):
    def __init__(self, hidden_size, output_size):
        ...
        self.mlp = MLP([hidden_size, output_size], activation='none')

    def forward(self, x):
        list_size = list(x.size())
        if len(list_size) > 2:
            x = x.view(-1, list_size[-1])
        x = self.mlp(x)
        list_size[-1] = x.size(-1)
        x = x.view(list_size)
        return x

class FusedDenseWrapper(nn.Module):
    def __init__(self, hidden_size, output_size):
        super(FusedDenseWrapper, self).__init__()
        self.fused_dense = FusedDense(hidden_size, output_size)

    def forward(self, x):
        list_size = list(x.size())
        if len(list_size) > 2:
            x = x.view(-1, list_size[-1])
        x = self.fused_dense(x)
        list_size[-1] = x.size(-1)
        x = x.view(list_size)
        return x
