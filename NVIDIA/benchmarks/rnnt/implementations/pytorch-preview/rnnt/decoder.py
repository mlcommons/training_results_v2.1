...
class RNNTGreedyDecoder:
    ...
    def _joint_step(self, model, enc, pred, f_g_len=None, log_normalize=False):
        logits = model.joint(enc, pred, f_g_len)[:, 0, 0, :]
    ...
    def _eval_main_loop_stream(self, label_tensor, hidden0, hidden1, time_idx, label_idx, complete_mask, num_symbol_added, num_total_symbol, current_label):
        x, out_len, arange_tensor, f_g_len = self.stashed_tensor
        ...
        # For eval, packing or not packing are the same because the seq len is 1.
        # Choosing to use not packing here for simplicity.
    ...
    def _greedy_decode_batch(self, x, out_len):
        ...
        while complete_mask.sum().item() != B:
            ...
            logp = self._joint_step(self.model, f, g, f_g_len, log_normalize=False)
            # get index k, of max prob
            v, k = logp.max(1)
            k = k.int()
            non_blank_mask = (k != self.blank_idx)
    ...
    def _capture_cg(self, x, out_len):
        ...
        self.stashed_tensor = x, out_len, arange_tensor, f_g_len
