...
seed_num=$(date +%s)
mkdir -p ${OUTPUT_DIR}
LOG=${OUTPUT_DIR}/dlrm_training_log_32k_1node/${PRECISION}_${seed_num}
rm -rf ${LOG}
mkdir -p ${LOG}
...
for (( I=0; I < NP ; I++)) ; do
  ...
  for((P=0;P < CCL_WORKER_COUNT ; P++)); do CCL_WORKER_AFFINITY="${CCL_WORKER_AFFINITY} $((I * CORES + P + NP * CORES))" ; done
...
TEST_BS=65536
MAX_RANGE=40000000
dlrm_extra_option="--hybrid-gradient-emb --ipex-merged-emb --bf16 --ipex-interaction --enable-mlp-fusion --allreduce-wait  " #--enable-profiling"
...
scontrol show hostname $SLURM_NODELIST >hostfile1
cat hostfile1
export MASTER_ADDR=$(head -1 hostfile1)
