...
if [ -z $MLPERF_ROOT_DIR ]; then
    # Assume we're running from /workspace/mosaicml if not specified
    MLPERF_ROOT_DIR="/workspace/mosaicml"
fi
...
BENCHMARK_CONFIG_PATH="${MLPERF_ROOT_DIR}/benchmarks/${BENCHMARK_NAME}/implementations/${SYS_ROOT_NAME}-${BENCHMARK_CONFIG}/config.yaml"
...
for run_index in $RUN_INDICES; do
    ...
    composer -n $SYS_GPU_NUM $ENTRYPOINT                                                \
        ...
        --callbacks.mlperf.exit_at_target "True"                                        \
        --seed $RUN_SEED                                                                \
        $WANDB_OPTS
