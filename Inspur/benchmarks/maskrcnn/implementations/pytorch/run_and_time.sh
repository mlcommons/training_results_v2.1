...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    if [[ "${DGXSYSTEM}" == NF5468M6J ]]; then
      CMD=( 'python' '-u' '-m' 'bind_launch_nf5468m6j' "--nsockets_per_node=${DGXNSOCKET}" \
        "--ncores_per_socket=${DGXSOCKETCORES}" "--nproc_per_node=${DGXNGPU}" )
    elif [[ "${DGXSYSTEM}" == NF5488A5 ]]; then
      CMD=( 'python' '-u' '-m' 'bind_launch_nf5488a5' "--nsockets_per_node=${DGXNSOCKET}" \
        "--ncores_per_socket=${DGXSOCKETCORES}" "--nproc_per_node=${DGXNGPU}" )
    elif [[ "${DGXSYSTEM}" == NF5688M6 ]]; then
      CMD=( 'python' '-u' '-m' 'bind_launch_nf5688m6' "--nsockets_per_node=${DGXNSOCKET}" \
        "--ncores_per_socket=${DGXSOCKETCORES}" "--nproc_per_node=${DGXNGPU}" )
    else
