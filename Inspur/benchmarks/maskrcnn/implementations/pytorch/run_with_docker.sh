...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PKLDIR}:/pkl_coco")
...
nvidia-docker run --rm --init --detach \
    -v $(cd -P $(dirname $0);pwd):/workspace/object_detection \
...
mllogger.mlperf_submission_log(mllogger.constants.MASKRCNN,org='Inspur',platform='NF5468M6J')"
