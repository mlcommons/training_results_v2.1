...
def add_dali_args(parser):
    ...
    group.add_argument('--dali-threads', type=int, default=3, help="number of threads" +\
                       "per GPU for DALI")
    group.add_argument('--dali-eval-threads', type=int, default=3, help="number of eval threads" +\
                       "per GPU for DALI")
    ...
    group.add_argument('--dali-eval-hw-decoder-load', type=float, default=0, help="Eval: Using H/W Jpeg Decode")
    group.add_argument('--dali-prefetch-queue', type=int, default=3, help="DALI prefetch queue depth")
    group.add_argument('--dali-eval-prefetch-queue', type=int, default=3, help="DALI eval prefetch queue depth")
...
class HybridTrainPipe(Pipeline):
    ...
    def __init__(self, batch_size, num_threads, device_id, rec_path, idx_path,
        ...
        if use_roi_decode and cache_size == 0:
            if version.parse(dali.__version__) >= version.parse("1.2.0"):
...
def build_input_pipeline(args, kv=None):
    ...
    valpipes = [HybridValPipe(batch_size     = batch_size,
                              num_threads    = args.dali_eval_threads,
                              device_id      = gpu_id,
                              ...
                              crop_shape     = target_shape[1:],
                              nvjpeg_padding = args.dali_nvjpeg_memory_padding * 1024 * 1024,
                              prefetch_queue = args.dali_eval_prefetch_queue,
                              seed           = args.seed,
                              dont_use_mmap  = True if args.dali_dont_use_mmap >=1
                                                    else False,
                              hw_decoder_load = args.dali_eval_hw_decoder_load,
                              resize_shp     = resize,
                              output_layout  = output_layout,
                              pad_output     = pad_output,
                              dtype          = args.dtype,
...
def get_rec_iter(args, trainpipes, valpipes, data_paths, kv=None):
    ...
    if num_workers == 1 or (args.profile > 0):
