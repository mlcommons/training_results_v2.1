...
echo "Using SCRIPTS as ${SCRIPTS} , DATADIR as ${DATADIR} , LOGDIR as ${LOGDIR} with ${_config_file} file"
_cont_mounts=("--volume=${SCRIPTS}:/workspace/dlrm" "--volume=${DATADIR}:/raid/datasets/criteo/mlperf/40m.limit_preshuffled/" "--volume=${LOGDIR}:${LOGDIR}")
sleep 30
...
for _experiment_index in $(seq 1 "${NEXP}"); do
  (
    if [[ $CLEAR_CACHES == 1 ]]; then
      ...
      docker exec -i "${_cont_name}" python3 -c "
from mlperf_logging.mllog import constants
from mlperf_logger.utils import log_event
log_event(key=constants.CACHE_CLEAR, value=True)"
    fi
    echo "Beginning trial ${_experiment_index} of ${NEXP}"
    docker exec -i ${_config_env[@]} ${_cont_name} bash ./run_and_time.sh
