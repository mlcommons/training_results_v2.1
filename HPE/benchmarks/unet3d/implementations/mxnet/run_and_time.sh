...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    cluster='champollion'
    CMD=( '/workspace/image_segmentation/bindpcie'  '--cpu=/workspace/image_segmentation/675D_cpu_topology.sh' '--ib=single' '--mem=/workspace/image_segmentation/675D_mem_topology.sh' '--' ${NSYSCMD} 'python' '-u')
else
