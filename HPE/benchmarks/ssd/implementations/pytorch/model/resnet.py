...
def _load_state_dict_from_file_or_url(url: str, progress: bool):
    filename = url.rsplit('/', 1)[-1]
    checkpoint = os.path.join("/workspace", "ssd", "backbone", filename)
    state_dict = None
    print(f"Checking for backbone file {checkpoint}")
    if os.path.exists(checkpoint):
        print(f"Loading backbone file {checkpoint}")
        return torch.load(checkpoint)
    else:
        print(f"Loading backbone URL {url}")
        return load_state_dict_from_url(url, progress=progress)
...
def _resnet(
    ...
    **kwargs: Any
    ...
    if pretrained:
        state_dict = _load_state_dict_from_file_or_url(model_urls[arch], progress)
