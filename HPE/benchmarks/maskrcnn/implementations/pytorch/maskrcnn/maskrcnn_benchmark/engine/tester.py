...
def test(cfg, model, distributed, shapes, eval_ranks_comm=None, dryrun=False, H_split=True):
    ...
    if cfg.DATALOADER.CACHE_EVAL_IMAGES:
        ...
        if _eval_datasets is None:
            data_loaders_val = make_data_loader(cfg, is_train=False, is_distributed=distributed, shapes=shapes, H_split=H_split)
            _eval_datasets = []
    ...
    if _first_test:
        ...
        mllogger.event(key=mllogger.constants.EVAL_SAMPLES, value=len(data_loaders_val), unique_log_rank=first_rank)
        _first_test = False
