...
if [[ "${KVSTORE}" == "horovod" ]]; then
    ...
    # BIND="./bind.sh --cpu=exclusive --ib=off --mem=node --cluster=${cluster} --"
    BIND="./bind.sh --cpu=sys-4124gs-tnr_nps4_cxx.sh --ib=off --mem=sys-4124gs-tnr_nps4_cxx.sh --cluster=${cluster} --"
fi
