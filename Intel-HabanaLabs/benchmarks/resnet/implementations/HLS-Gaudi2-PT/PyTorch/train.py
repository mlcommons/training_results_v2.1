...
def load_data(traindir, valdir, args, synthetic=False):
    # Data loading code
    print("Loading data")
    ...
    dataset_loader_train = habana_dataloader.habana_dataset.ManifestDatasetInfo if args.dl_worker_type == "HABANA" and not synthetic else torchvision.datasets.ImageFolder
    ...
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset) if args.dl_worker_type != "HABANA" else None
...
def main(args):
    ...
    steps_per_epoch = ceil(NUM_IMAGES['train'] / args.workers / args.batch_size)
    steps_per_eval = ceil(NUM_IMAGES['validation'] / args.workers / args.batch_size)
    ...
    manifest_path = prepare_dataset_manifest(args)
    ...
    if args.dl_worker_type == "HABANA":
        data_loader = data_loader_type(
            dataset, batch_size=args.batch_size, shuffle=True, sampler=train_sampler,
            num_workers=args.workers, pin_memory=pin_memory, pin_memory_device=pin_memory_device, manifest_path=manifest_path)
    else:
        data_loader = data_loader_type(
...
def prepare_dataset_manifest(args):
    ...
    manifest_file_name = f"imagenet_jpeg_manifest_rank_{utils.get_rank()}.json"
...
def parse_args():
    ...
    parser.add_argument('--base_learning_rate', default='9', type=float, help='initial learning rate')
    ...
    parser.add_argument('--hmp-bf16', default='',
                        help='path to bf16 ops list in hmp O1 mode')
    parser.add_argument('--hmp-fp32', default='',
