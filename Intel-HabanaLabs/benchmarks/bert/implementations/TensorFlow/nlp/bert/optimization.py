...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, manual_fp16=False, use_fp16=False, num_accumulation_steps=1,
                     optimizer_type="adam", allreduce_post_accumulation=False, init_loss_scale=2**32, weight_decay_rate=0.01,
                     beta_1=0.9, beta_2=0.999, epsilon=1e-6, power=0.5, use_tpu=False):
  ...
  elif optimizer_type == "sharded_lamb":
    optimizer = ShardedLAMBOptimizer(
        learning_rate=learning_rate,
        weight_decay_rate=weight_decay_rate,
        beta_1=beta_1,
        beta_2=beta_2,
  ...
  if use_fp16:
    ...
    optimizer = tf.compat.v1.mixed_precision.enable_mixed_precision_graph_rewrite(optimizer, loss_scaler)
  ...
  if num_accumulation_steps > 1:
    ...
    accum_vars = tf.cond(pred=reset_step,
                         true_fn=lambda: [accum_vars[i].assign(tf.cast(grad, accum_vars[i].dtype)) for i, grad in enumerate(clipped_grads)],
                         false_fn=lambda:  [accum_vars[i].assign_add(tf.cast(grad, accum_vars[i].dtype)) for i, grad in enumerate(clipped_grads)])
    ...
    def allreduce_of_batch_finite_required():
      ...
      return hvd is not None and hvd.is_initialized() and manual_fp16 and use_fp16
    ...
    def update(accum_vars):
      with tf.control_dependencies([global_step.assign(new_global_step)]):
        if allreduce_post_accumulation and hvd is not None and hvd.is_initialized():
          accum_vars = [hvd.allreduce(tf.convert_to_tensor(value=accum_var)* 1.0 / num_accumulation_steps, op=hvd.Average) if isinstance(accum_var, tf.IndexedSlices)
                        else hvd.allreduce(accum_var * 1.0 / num_accumulation_steps, op=hvd.Average) for accum_var in accum_vars]
  ...
  def apply_gradients(self, grads_and_vars, global_step, name=None,
    ...
    for (grad, param) in grads_and_vars:
      ...
      next_v = (
          tf.multiply(self.beta_2, v) + tf.multiply(1.0 - self.beta_2,
                                                    tf.square(grad)))
      beta1_correction = tf.math.reciprocal(1 - self.beta_1 ** steps)
      beta2_correction = tf.math.reciprocal(1 - self.beta_2 ** steps)
      next_m_unbiased = next_m * beta1_correction
      next_v_unbiased = next_v * beta2_correction
      update = next_m_unbiased * tf.math.rsqrt(next_v_unbiased + self.epsilon*self.epsilon)
      # Just adding the square of the weights to the loss function is *not*
      # the correct way of using L2 regularization/weight decay with Adam,
      # since that will interact with the m and v parameters in strange ways.
      #
...
class ShardedLAMBOptimizer(LAMBOptimizer):
  """Distributed version of LAMB optimizer that includes "correct" L2 weight decay."""
  def __init__(self,
               learning_rate,
               weight_decay_rate=0.0,
               beta_1=0.9,
               ...
               epsilon=1e-6,
               exclude_from_weight_decay=None,
               name="ShardedLAMBOptimizer"):
    """Constructs a ShardedLAMBOptimizer."""
    super().__init__(learning_rate,
          weight_decay_rate, beta_1, beta_2, epsilon, exclude_from_weight_decay, name)

  def apply_gradients(self, grads_and_vars, global_step, name=None):
    """
    Distributed version of apply_gradients.
    Modifies LAMB optimizer by dividing the parameters and gradients to two groups:
    1) Non-divisible - the first dimension of tensor is not divisible by the number of workers:
    The parameters and gradients are updated with standard LAMB update.
    2) Divisible - the first dimension of tensor is divisible by the number of workers
    The parameters and gradients are 'sliced' and update is computed only to the slice corresponding to the worker.
    Updates are allgathered and original shapes reconstructed before the assingment of new weight values.
    """
    assignments = []
    steps = tf.cast(global_step, tf.float32)
    num_grads = len(grads_and_vars)

    grads = []
    params = []
    param_holders = [] # holds param variables
    is_divisible = []
    for i, gv in enumerate(grads_and_vars):
      grads.append(gv[0])
      params.append(gv[1])
      param_holders.append(gv[1])
      if gv[0].shape[0] % hvd.size() == 0:
        is_divisible.append(True)
      else:
        is_divisible.append(False)

    def chunk_size(grad_or_param):
      return grad_or_param.shape[0] // hvd.size()

    def get_start_offset(grad_or_param):
      return hvd.rank() * chunk_size(grad_or_param)

    def get_end_offset(grad_or_param):
      return (hvd.rank() + 1) * chunk_size(grad_or_param)

    grads = [grad[get_start_offset(grad) : get_end_offset(grad)] if is_divisible[i] is True
             else grad for i, grad in enumerate(grads)]

    params = [param[get_start_offset(param) : get_end_offset(param)] if is_divisible[i] is True
              else param for i, param in enumerate(params)]

    # divisible update
    for i in range(num_grads):
      is_shape_divisible = is_divisible[i]
      grad = grads[i]
      param = params[i]
      param_holder = param_holders[i]
      if grad is None or param is None:
        continue
      param_name = self._get_variable_name(param_holder.name)
      ...
      ratio = 1.0
      if self._do_layer_adaptation(param_name):
        if is_shape_divisible:
           w_norm = tf.reduce_sum(param*param, name="local_weight_sum_squares")
           g_norm = tf.reduce_sum(update*update, name="local_gradient_sum_squares")

           # allreduce squared norms according all ranks
           w_norm = tf.sqrt(hvd.allreduce(w_norm, op=hvd.Sum), name="w_norm")
           g_norm = tf.sqrt(hvd.allreduce(g_norm, op=hvd.Sum), name="g_norm")
        else:
           w_norm = linalg_ops.norm(param, ord=2, name="w_norm")
           g_norm = linalg_ops.norm(update, ord=2, name="g_norm")
      ...
      if is_shape_divisible:
        next_param = hvd.allgather(next_param)

      assignments.extend(
        [param_holder.assign(next_param),
         m.assign(next_m),
         v.assign(next_v)])
