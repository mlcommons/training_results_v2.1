...
from TensorFlow.nlp.bert.utils.lightweight_checkpoint_saver import LightweightCheckpointSaver
...
from TensorFlow.nlp.bert.utils.async_checkpoint import AsyncCheckpointSaverHook
...
def init_flags():
  ...
  flags.DEFINE_string(
      "optimizer_type", "lamb",
      "Optimizer used for training - 'lamb', 'sharded_lamb' or 'adam'")
  ...
  flags.DEFINE_bool("experimental_slack", True, "Enable experimental slack")
  ...
  flags.DEFINE_integer("num_dist_eval_workers", 0,
                        "Number of workers to participate in distributed evaluation")
  ...
  flags.DEFINE_string(
      ...
      "Lightweight checkpoint implementation 'basic' or 'sharded'. 'basic' stores all TF Variables in file for each "
      "rank, while 'sharded' stores only a subset of TF Variables and the remianing variables are exchanged with other "
      "ranks via Horovod_Allreduce.")

  flags.DEFINE_bool(
      'use_async_checkpointing', True,
      'If true then checkpoints are stored in asynchronous way (in separate thread).')

  flags.DEFINE_string("profile_steps", None,
    'Save profiling data to model dir at given range of global steps. The '
    'value must be a comma separated pair of positive integers, specifying '
    'the first and last step to profile. For example, "--profile_steps=2,4" '
    'triggers the profiler to process 3 steps, starting from the 2nd step.')

  flags.DEFINE_bool("log_session_run_hook", False, "Whether to enable hook for logging with dllogger")
  flags.DEFINE_bool("examples_per_second_hook", False, "Whether to enable hook for logging performance")
  flags.DEFINE_bool("loop_unrolling_for_train_op", True, "Whether to enable loop unrolling for train op. This feature removes the need to have switch/merge operations in train_op.")

def parse_profile_steps(profile_steps):
  profile_steps_parsed = [int(i) for i in profile_steps.split(',')]
  assert len(profile_steps_parsed) == 2, f"Profile steps (profile_steps) format is invalid. Expected: <START_STEP>,<STOP_STEP>, got: {profile_steps}."

  start_step, stop_step = profile_steps_parsed
  assert start_step >= 0 and start_step <= stop_step, "Both start and stop steps have to be positive numbers and the "\
                                                      "stop step has to be grater than or equal to the start step."
...
def get_mllog_mlloger():
    ...
    if not hasattr(get_mllog_mlloger, attr):
      ...
      str_hvd_rank = str(hvd.rank()) if hvd is not None and hvd.is_initialized() else "0"
      mllogger = mllog.get_mllogger()
      log_dir = FLAGS.output_dir
      if "LOG_DIR" in os.environ:
          log_dir = os.environ["LOG_DIR"]
      filenames = os.path.normpath(log_dir) + "/result_rank_" + str_hvd_rank + ".txt"
...
class _LogSessionRunHook(tf.estimator.SessionRunHook):
  ...
  def before_run(self, run_context):
    if hvd is not None and hvd.is_initialized() and hvd.rank() != 0:
      return
    self.t0 = time.time()
...
class MLPerfHook(tf.estimator.SessionRunHook):
  def __init__(self, global_batch_size, num_accumulation_steps, num_train_steps, samples_between_eval,
    ...
    '''
       global_batch_size = train_batch_size * num_accumulation_steps * avg_seq_per_pack * num_of_devices
    ...
    mllogger.event(key=mllog.constants.EVAL_SAMPLES, value=EVAL_SAMPLES)
  ...
  def after_run(self, run_context, run_values):
    if self.warmup_step:
      if self.num_accumulation_steps <= 1:
        ...
        _, update_step = run_values.results
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      if is_training:
        ...
        if hvd is not None and hvd.is_initialized() and hvd.rank() == 0:
          for var in tvars:
            mllogger.event(key=mllog.constants.WEIGHTS_INITIALIZATION, metadata={'tensor': var.op.name})
    ...
    if FLAGS.profile_steps:
      tf_profiler_hook = RangeTFHltvProfilerHook(
        "./rank-{}".format(hvd.rank() if hvd.is_initialized() else 0),
        FLAGS.profile_steps
        )
    ...
    if FLAGS.use_lightweight_checkpoint:
      light_checkpoint_saver =  LightweightCheckpointSaver(
        ...
        comm_rank=comm_rank,
        comm_size=comm_size,
        variables_caching=FLAGS.use_async_checkpointing,
        sharded=False,
        max_to_keep=20,
        defer_build=False,
        save_relative_paths=True)

      saver_scaffold = tf.compat.v1.train.Scaffold(saver=light_checkpoint_saver)
      ...
      if save_checkpoints_steps:
        if FLAGS.use_async_checkpointing:
          tf.compat.v1.logging.info("AsyncCheckpointSaverHook will be used for checkpoint saving")

          saver_hook = AsyncCheckpointSaverHook(
            checkpoint_dir = FLAGS.output_dir,
            save_secs = None,
            save_steps = save_checkpoints_steps,
            scaffold = saver_scaffold,
          )
          training_hooks.append(saver_hook)

          # checkpoint saving is performed by saver hook, so scaffold has to be disabled
          training_saver_scaffold = None
        else:
          tf.compat.v1.logging.info("CheckpointSaverHook will be used for checkpoint saving")

    if mode == tf.estimator.ModeKeys.TRAIN:
      global_step = tf.compat.v1.train.get_or_create_global_step()
      if FLAGS.loop_unrolling_for_train_op:
        from training_ops import create_training_ops
        ops_list, train_op = create_training_ops(global_step,
        ...
        output_spec = EstimatorSpec(
            ...
            scaffold=training_saver_scaffold
        )
      else:
...
def input_fn_builder(input_files,
  ...
  def input_fn():
    ...
    if is_training:
      ...
      if hvd is not None and hvd.is_initialized(): d = d.shard(hvd.size(), hvd.rank())
      d = d.repeat()
      ...
      if hvd is not None and hvd.is_initialized() and FLAGS.is_dist_eval_enabled:
        # In order to perform accurate distributed evaluation we perform as follwoing:
        # We divide the dataset into number of shards that is smaller or equal to the number of workers
        # In a way that all of the examples will be present in the dataset
        num_shards = FLAGS.num_dist_eval_workers

        if hvd.rank() < FLAGS.num_dist_eval_workers:
          current_shard = hvd.rank()
        else:
          # If the worker will not be used in evaluation, we assign shard rank % num_shards to it.
          # It's evaluation results will be sipped drung evaluation.
          current_shard = hvd.rank() % FLAGS.num_dist_eval_workers

        d = d.shard(num_shards, current_shard)
    ...
    options.experimental_slack=FLAGS.experimental_slack
...
def main(_):
  ...
  # In multi-node scenario, workers stores data in separate directories. However when basic lightweight checkpoints are
  # used then checkpoints are stored only by worker with local rank 0, therefore other workers during evaluation take
  # checkpoints from this worker working directory. When shareded lightweight checkpoints are used then each worker
  # stores and next reads checkpoints from its own working directory.
  if hvd is not None and hvd.is_initialized():
    ...
    if not FLAGS.use_lightweight_checkpoint or FLAGS.lightweight_checkpoint_impl == "basic":
      ...
      raise ValueError("Not supported configuration")
  else:
    checkpoints_location = FLAGS.output_dir
...
#    config.gpu_options.per_process_gpu_memory_fraction = 0.7
  ...
  if FLAGS.optimizer_type in ["lamb", "sharded_lamb"]:
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.examples_per_second_hook:
    ...
    chpt_file_path = checkpoints_location + "/checkpoint"
    chpt_files = []
      ...
      with open(chpt_file_path, "r") as file:
          for line in file:
            ...
            if tmp == 'all_model_checkpoint_paths':
              ...
              if step >0:
                chpt_files.append(checkpoints_location + '/'+ chpt_step.strip().strip('"'))
    eval_files = []
    ...
    if len(chpt_files) == 0:
      ...
      if hvd is not None and hvd.is_initialized() and FLAGS.is_dist_eval_enabled:
        ...
        # Validate the distributed evaluation configuration
        assert (FLAGS.max_eval_steps * FLAGS.num_dist_eval_workers * FLAGS.eval_batch_size) == EVAL_SAMPLES
        eval_samples = FLAGS.max_eval_steps * FLAGS.eval_batch_size
        ...
        for ckpt_ind,chpt_path in enumerate(chpt_files):
          ...
          if FLAGS.stop_threshold:
            if hvd.rank() < FLAGS.num_dist_eval_workers:
              ...
              partial_eval_masked_lm_accuracy = eval_results["masked_lm_accuracy"] * 0
            ...
            total_eval_masked_lm_accuracy_FP32 /= EVAL_SAMPLES
...
if __name__ == "__main__":
  ...
  if FLAGS.enable_habana_backend:
    ...
    os.environ['TF_CLUSTER_VARIABLES']='true'
    load_habana_module()
