...
def create_pretraining_dataset(input_file, max_pred_length, shared_list, args, worker_init_fn, synthetic_input=False):
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler,
        batch_size=args.train_batch_size, num_workers=0 if args.train_batch_size<=8 else args.num_workers, worker_init_fn=worker_init_fn,
...
def create_eval_dataset(args, worker_init_fn):
    ...
    eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size,
                                         num_workers=0 if min(chunk_size, args.eval_batch_size)<=10 else args.num_workers, worker_init_fn=worker_init_fn, pin_memory=True)
...
#            assert use_DDP, "Running multi-GPU without --use_ddp"
        ...
        while global_step < args.max_steps and not end_training:
            ...
            train_sampler = RandomSampler(train_data)
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                          batch_size=args.train_batch_size, num_workers=0 if args.train_batch_size<=8 else args.num_workers, worker_init_fn=worker_init, pin_memory=True,drop_last=True)
